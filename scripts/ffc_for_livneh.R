...
clean_df <- read_csv("data_raw/fnf_daily_livneh_TUO_1950_2013.csv") %>% 
...
clean_df_ffc <- clean_df %>%
  select(flow=flow, date) %>% as.data.frame()
...
ffc$step_one_functional_flow_results(timeseries = clean_df_ffc,
                                     comid=COMID,
                                     token = ffctoken,
                                     output_folder = "output/ffc_livneh")

# Look at Results ---------------------------------------------------------

ffc$ffc_percentiles %>% View()
ffc$ffc_results %>% View()
write_csv(ffc$predicted_percentiles, file = glue("output/ffc_livneh/{COMID}_livneh_ffc_predicted_percentiles.csv"))
...
# write out 
write_csv(ann_metrics, file = glue("output/ffc_livneh/{COMID}_livneh_ffc_ann_metrics.csv"))
